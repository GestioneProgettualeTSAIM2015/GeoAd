$(function () {
    ...
    loginForm.submit(function (event) {
        ...
        var token = localStorage.getItem(headToken + user);
        if (token != null)
            logInAccount(token);
        else {
    ...
    function saveToken(user, token) {
        localStorage.setItem(headToken + user, token);
    ...
    $("#btnGeneric").on("click", function () {
        $.ajax({
            ...
            error: function (xhr) {
                ...
                display.text("Errore nel recupero dei dati: " + xhr.responseText);
            }
        });
    });
    ...
    if (localStorage.length > 0) {
        var token = localStorage.getItem(localStorage.key(0));
        logInAccount(token);
    }
});